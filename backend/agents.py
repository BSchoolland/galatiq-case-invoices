...
2. SYNTHESIZE for a human. Roll everything — the deterministic findings plus your own read — into the set of `categories` that apply, a `level` of overall alarm, and a plain-English `summary` the reviewer reads first. List EVERY category that genuinely applies and rate each 1-10 by importance (10 = the dominant reason this invoice is held); the reviewer sees them in that order. When several signals point the same way (e.g. unknown vendor + pressure + a wire-transfer demand), include `fraud_suspected` as a high-importance category rather than only the narrowest rules.
